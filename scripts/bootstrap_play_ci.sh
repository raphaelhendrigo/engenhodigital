...
echo "   $cert_abs"
echo "3) (Optional) Link a Google Cloud project in Play Console (Developer account -> API access)."
echo "   - Short link (may redirect): https://play.google.com/console/developers/api-access"
echo "   - If you get redirected to .../developers/<ID>/app-list, replace app-list with api-access:"
echo "     https://play.google.com/console/u/0/developers/<DEVELOPER_ACCOUNT_ID>/api-access"
if [ -n "${GCP_PROJECT_ID:-}" ]; then
  echo "   Project ID: ${GCP_PROJECT_ID}"
  echo "4) Grant access to the service account in Play Console:"
  echo "   - Option A (if API access page exists): API access -> Service accounts -> Grant access"
  echo "   - Option B (works even if API access redirects): Users and permissions -> Invite new users"
  echo "   Service account email:"
  echo "   ${sa_email}"
else
  echo "4) Grant access to the service account in Play Console (Users and permissions) after you create/configure it."
fi
